...
💡 *Action:* "Set ${pair} at [rate]"`;
  ...
  async handleSetAlert(args, phoneNumber, database, priceService) {
    ...
    try {
      let user = database.getUserByPhoneNumber(phoneNumber);
      if (!user) user = database.createUser(phoneNumber, phoneNumber);

      // Get current count before adding (to show user which number this is)
      const existingAlerts = await database.getUserAlerts(user.id);
      const newAlertNumber = existingAlerts.length + 1; // It will be next number
...
� Type "My alerts" to see your list.`;
  ...
  async handleMyAlerts(args, phoneNumber, database, priceService) {
    ...
    alerts.forEach((alert, index) => {
      const formattedTarget = priceService.formatPrice(alert.target_price, alert.asset);
  ...
  async handleDeleteAlert(args, phoneNumber, database, priceService) {
    ...
    // 1. Get ALL alerts for this user
    ...
    // 3. Delete using the REAL Database ID
    const result = await database.deleteAlert(targetAlert.id, user.id);
    
    if (result.changes === 0) return `❌ System Error: Could not delete alert.`;
...
Removed: ${targetAlert.asset} (Alert #${userIndex})`;
  ...
  async handleStatus(args, phoneNumber, database, priceService) {
    ...
    if (!user) return '👋 Send "Price BTC" to start.';
    const alerts = await database.getUserAlerts(user.id);
    return `📊 *Status*: ${user.subscription_type.toUpperCase()} | Active Alerts: ${alerts.length}`;
  }
...
Example: "Name John Doe"`;
    ...
    try {
      const success = database.updateUserName(phoneNumber, newName);
      if (success) {
