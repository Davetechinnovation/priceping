...
class PriceService {
  constructor() {
    ...
    // 🟢 ADD HEADERS TO LOOK LIKE A BROWSER (Fixes API blocks)
    this.headers = {
  ...
  async getAssetInfo(input) {
    // 1. CLEAN INPUT (Remove slashes for GBP/USD -> GBPUSD)
    const cleanInput = input.toUpperCase().replace(/\//g, "").trim();
    const parts = cleanInput.split(" ");
    let symbol = parts[0];
    let specificChain = parts[1] || null;
    ...
    // Manual Mappings for tricky coins
    ...
    // Fuzzy Match (if exact not found, e.g. "BITCOIN")
    if (!options) {
        // Try to find if user typed full name "BITCOIN" instead of "BTC"
        // This is expensive, so only do if undefined
    }

    if (options) {
        // 🟢 PRIORITY SORTING (Fixes the HarryPotter Issue)
        // We force "Bitcoin" blockchain to be #1 for BTC, "Ethereum" for ETH
        ...
        // 🛡️ ANTI-SPAM HARD FILTER
        // If searching for BTC, ONLY accept the real Bitcoin chain as default
        if (symbol === 'BTC') {
        ...
        // Specific Chain Request? (e.g. "Price USDT TRON")
        if (specificChain) {
            const match = options.find(o => o.blockchain.toUpperCase().includes(specificChain));
    ...
    // 💱 4. CHECK FOREX
    // Only verify if looks like currency (3 chars like JPY or 6 chars like EURUSD)
  ...
  formatPrice(price, symbol) {
      ...
      // Forex formatting (usually 4-5 decimals)
  ...
  async getForexPrice(pair) {
    // Ignore Crypto confusing lookalikes
